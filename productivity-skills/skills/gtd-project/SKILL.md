...
2. **For overdue reschedule** (arrived from auto-infer):
   ...
   - Use **AskUserQuestion** for a new deadline or whether to remove the date if it was only a planning target. Use a date already supplied in the request without asking again.
...
4. Update the selected reminder in place using its `id` from the query. Pass only changed fields:
   ```bash
   swift ${CLAUDE_PLUGIN_ROOT}/scripts/productivity-cli.swift reminders update \
     --id "<action-id>" \
     --title "New title" \
     --list "@new-list"
   ```
