...
- If 5, use **AskUserQuestion** to ask for the custom model ID and use that value exactly.
...
- Otherwise, use the current `model` value from settings.json. If no `model` key exists, treat as unknown (present all levels with a note that validity depends on the model).

If the reference model is `claude-haiku-4-5-20251001`, inform the user: "Haiku 4.5 does not support effortLevel. Skipping effort configuration." and proceed to Step 6.

If the reference model is unknown or a custom ID, present all levels and note that validity depends on the model.

Otherwise, use **AskUserQuestion** to present the valid effort levels for the reference model:
...
**Known bug:** `max` does not persist correctly in `settings.json` due to a serialization issue in Claude Code. If the user selects `max`, warn them:

> "Note: `max` effort has a known bug where it does not persist in settings.json. As a workaround, add `export CLAUDE_CODE_EFFORT_LEVEL=max` to your shell profile (~/.zshrc or ~/.bashrc) instead. Would you like to proceed with writing `max` to settings.json anyway, or skip?"

If the user chooses to skip, do not write the `effortLevel` key.
...
"Enter desired auto-compact percentage (1–83). Values above 83 are capped internally."
...
Read `~/.claude/settings.json` again (to get the latest state), then use the **Edit** tool to apply all collected changes in a single surgical update:

- If `~/.claude/settings.json` does not exist, create it with only the keys being set.
...
- For `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`: if no `env` block exists, add one. Add or replace only the `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` key within `env`. Preserve all other keys in both the `env` block and the rest of the file. The value must be a **string**.
